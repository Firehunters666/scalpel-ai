...
# -------------------------------------------------------------------
# CUSTOM CSS FOR CLEANER ENTERPRISE DASHBOARD
# -------------------------------------------------------------------
st.markdown("""
<style>
    /* Main container */
    ...
    /* Metric cards – lighter, more compact */
    .metric-card {
    ...
    /* Sidebar navigation buttons – more compact */
...
# -------------------------------------------------------------------
# DASHBOARD METRICS & CHARTS (refined UI)
# -------------------------------------------------------------------
def display_dashboard(tb, revenue_df, net_income, monthly_df):
    ...
    # Comparisons (if monthly data available)
    prev_revenue = None
    ...
    # Metric row (5 columns) – no greeting
    ...
    with col1:
        ...
        """, unsafe_allow_html=True)
    with col2:
        compare_text = f"vs prev: ${(total_revenue - prev_revenue):+,.0f}" if prev_revenue else ""
        st.markdown(f"""
        <div class="metric-card">
        ...
        """, unsafe_allow_html=True)
    with col3:
        compare_text = f"vs prev: ${(net_income - prev_net_income):+,.0f}" if prev_net_income else ""
        st.markdown(f"""
        <div class="metric-card">
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">${ar_balance:,.0f}</div>
        </div>
        """, unsafe_allow_html=True)
    with col5:
        st.markdown(f"""
        <div class="metric-card">
            ...
            <div class="metric-value">${ap_balance:,.0f}</div>
        </div>
        """, unsafe_allow_html=True)

    # Workflow Snapshot as a clean table
    st.markdown("### Workflow Snapshot")
    workflow_data = {
        "Cash Transactions to be reconciled": "13",
        "Journal entries pending approval": "2",
        "Invoices to be sent": "809",
        "Bills to be paid": "229",
        "Contracts to be reviewed": "2",
        "Last closed month": "May 2024"
    }
    workflow_df = pd.DataFrame(list(workflow_data.items()), columns=["Account", "Description"])
    ...
    with tech_cols[1]:
        st.image("https://cdn.jsdelivr.net/gh/devicons/devicon/icons/salesforce/salesforce-original.svg", width=30)
        st.caption("Salesforce")
        st.caption("Connected")
    with tech_cols[2]:
        st.image("https://cdn.jsdelivr.net/gh/devicons/devicon/icons/stripe/stripe-original.svg", width=30)
        st.caption("Stripe")
        st.caption("Connected")
    with tech_cols[3]:
        st.caption("Avatax")
    ...
    with tech_cols[4]:
        ...
        st.caption("Set up")
    st.markdown("---")
    st.markdown("#### Browse integrations")
    ...
    with report_cols[0]:
        if st.button("Income Statement", use_container_width=True):
            st.session_state.current_page = "Profit & Loss"
            st.rerun()
    with report_cols[1]:
        if st.button("Revenue Waterfall", use_container_width=True):
            st.info("Revenue Waterfall report – coming soon.")
    with report_cols[2]:
        if st.button("Datalab", use_container_width=True):
            st.info("Datalab – coming soon.")
    with report_cols[3]:
        if st.button("Prepaid Schedule", use_container_width=True):
            st.info("Prepaid Schedule – coming soon.")

    # Charts Section (optional, keep as before)
    ...
    if monthly_df is not None:
        ...
        with col1:
            ...
            fig_rev.update_layout(template='plotly_white', hovermode='x unified')
            st.plotly_chart(fig_rev, use_container_width=True)
        with col2:
            fig_margin = px.line(monthly_df, x='year_month', y=['Gross Margin %', 'Net Margin %'],
                                 title="Gross & Net Margin %", markers=True)
            fig_margin.update_layout(template='plotly_white', hovermode='x unified')
        ...
        fig_bar = px.bar(top_rev, x='account_description', y='amount', title="Top Revenue Streams",
                         text='amount', template='plotly_white')
...
# -------------------------------------------------------------------
# MAIN APP (with button-based sidebar navigation)
# -------------------------------------------------------------------
def main():
    ...
    pages = ["Dashboard", "Account Ledgers", "Trial Balance", "Profit & Loss", "Balance Sheet",
    ...
    st.sidebar.markdown("---")
    # Optional user name input (kept but not displayed anywhere)
    ...
    if selected == "Dashboard":
        ...
        (current_assets, non_current_assets, total_current_assets, total_non_current_assets, total_assets,
         current_liabilities, non_current_liabilities, total_current_liabilities, total_non_current_liabilities,
         total_liabilities, equity_df, total_equity) = generate_balance_sheet(tb, net_income)
        ...
        if abs(total_assets - (total_liabilities + total_equity)) < 0.01:
            ...
            st.warning(
                f"⚠️ Imbalance of {total_assets - (total_liabilities + total_equity):,.2f} (automatically corrected).")
